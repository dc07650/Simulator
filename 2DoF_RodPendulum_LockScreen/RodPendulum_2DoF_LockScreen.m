...
th(1,1) = 80*pi/180;  th(1,2) = 120*pi/180; % Initial Values
...
%     plot(x(k,3),y(k,3),'rs'); hold on; % Joint 3
    plot([x(k,1) x(k,2)],[y(k,1) y(k,2)],'w','linewidth',2); hold on; % Link 1
    plot([x(k,2) x(k,3)],[y(k,2) y(k,3)],'w','linewidth',2); hold on; % Link 2
    if k < kp
        plot(x(1:k,3),y(1:k,3),'c','linewidth',0.5);            % End Point
    else
        plot(x((1+k-kp):k,3),y((1+k-kp):k,3),'c','linewidth',0.5);            % End Point
    end
    axis([-1 1 -19/9 19/9])                         % Axis X, Y
